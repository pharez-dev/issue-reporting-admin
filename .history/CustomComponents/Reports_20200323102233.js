      ...
      const images =
        this.state.data && this.state.data.images.length > 0
          ? this.state.data.images.map(image => {
              image = image.replace("/upload/", "/upload/h_480,q_auto,f_auto/");
              let thumbnail = image.replace(
                "/upload/",
                "/upload/h_360,q_auto,f_auto/"
              );

              return { original: image, thumbnail: thumbnail };
            })
          : null;
      return (
        <Modal
        ...
        >
          <div>
            {this.state.data && this.state.data.images.length > 0 && (
              <div>Put new image slider here</div>
            )}
          </div>
