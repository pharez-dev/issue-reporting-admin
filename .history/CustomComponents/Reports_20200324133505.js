  ...
  class extends React.Component {
    ...
    fetch = async () => {
      try {
        ...
        this.fetchWards(data.data.county, data.data.sub_county);
      } catch (err) {
        console.error(err);
    ...
    fetchWards = async (county, constituency) => {
      county = county.slice(0, county.indexOf("County") - 1).replace(" ", "+");

      try {
        const get = await fetch(
          `https://cors-anywhere.herokuapp.com/https://frozen-basin-45055.herokuapp.com/api/wards?county=${county}`,
          {
            method: "GET"
            // headers: {
        ...
        let wards = await get.json();
        wards = wards.filter(ward => {
          return ward.constituency == constituency;
        });

        console.log("wards", data.length, wards.length);
        this.setState({
          loading: false,
          ward
        });
