...
const MainHeader = ({ global, router }) => {
  ...
  useEffect(() => {
    ...
    if (!listening) {
      ...
      try {
        reqwest({
          url: `${globals.BASE_URL}/api/admin/header_notifications`,
          method: "post",
          headers: {
            Authorization: token,
          },
          ...
          let notif = Array.from(data.data);
          ...
          setNotificatons((notifications) => [...notifications, ...notif]);
          // this.setState({
          //   loading: false,
          //   data: data.issues,
          //   pagination
        ...
        <Link href="/">
          <a className="brand">
            ...
            <strong className="mx-1 text-black">
              <span style={{ textTransform: "uppercase" }}></span> Real Time
              County Issue-Reporting System
            </strong>
          </a>
        </Link>
