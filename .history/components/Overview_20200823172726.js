    ...
    <div>
      ...
      <Row gutter={16}>
        ...
        <Col sm={24} md={12} className="mb-4">
          ...
          >
            <Timeline pending="..." className="mt-2">
              {latest.map((each) => {
                let color = "purple";
                switch (each.type) {
                ...
                return (
                  <Timeline.Item
                    key={each._id}
                    dot={<MdAdjust size={"1.5em"} style={{ color: color }} />}
                  >
                    <div className="text-truncate">
                      <TimelinePeriod
                        content={moment(each.createdAt).format(
                          "MMMM Do YYYY, h:mm a"
                        )}
                      />
                      <span>{each.type}</span>
        ...
        <Col sm={24} md={12} className="mb-4">
          <Card
            title="Counties with most Issues"
            extra={
              <Dropdown overlay={menu}>
                <MoreHorizontal
                  size={20}
